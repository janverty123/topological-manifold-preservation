...
- `configs/config_split_mnist_5task.yaml` — same architecture and starting hyperparameters as your validated 2-task config
...
- `configs/config_split_mnist_5task.yaml` — added `tmp.ema_alpha` and
...
model (`outputs_split_mnist_5task/models/task0_base_model.pt`), so only
the first run pays the pretraining cost.

## Important — don't assume your 2-task hyperparameters transfer unchanged

`configs/config_split_mnist_5task.yaml` starts `tmp.lambda_` and
...
`outputs_split_mnist_5task/logs/<method>.jsonl` gives you, per epoch:
...
```bash
python diagnose_gradient_conflict.py --config configs/config_split_mnist_5task.yaml --task-idx 2
```
...
`configs/config_split_mnist_5task.yaml`) instead of every single step.
...
```bash
python diagnose_task_confusion.py --config configs/config_split_mnist_5task.yaml --method tmp --victim-task <N>
```
