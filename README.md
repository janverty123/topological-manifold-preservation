...
- If accuracy plateaus below 95% before `task1_max_epochs` (30) is
  ...
  best model reached — increase `task1_max_epochs` in `configs/config.yaml`
...
(`λ = 400` by default, in `configs/config.yaml`).
...
| **Limitations** — restricted to H0/H1 persistent homology | `tda.homology_dims: [0, 1]` in `configs/config.yaml` |
...
- [ ] `configs/config.yaml` reviewed (seed = 42 by default for exact reproducibility)
